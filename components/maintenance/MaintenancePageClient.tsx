...
function getNextJanuaryFirstIso() {
  const now = new Date();
  const endOfJan1 = new Date(now.getFullYear(), 0, 1, 23, 59, 59, 999);
  const targetYear = now > endOfJan1 ? now.getFullYear() + 1 : now.getFullYear();
  return `${targetYear}-01-01T00:00:00+01:00`;
}

const LAUNCH_AT = getNextJanuaryFirstIso();
...
function BigCountdown({ target }: { target: string }) {
  ...
  const launchLabel = useMemo(() => {
    ...
    if (isNaN(date.getTime())) return "bientôt";
  ...
  return (
    <div className="mt-3">
      <p className="text-[11px] sm:text-xs text-[#7e3b54] tracking-wide">
        Ouverture prévue le {launchLabel}
      </p>
      <div
    ...
    <main className="min-h-screen bg-gradient-to-b from-[#FFE5F4] via-[#F9BDD9] to-[#EC7EB8] text-[#2b1019] p-4 sm:p-6">
      ...
      {lightbox !== null && (
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby="lightbox-brand-title"
        ...
        >
          ...
          >
            <div className="absolute top-4 left-1/2 -translate-x-1/2 z-20 rounded-full bg-white/90 px-4 py-2 text-xs font-semibold tracking-[0.28em] text-[#2b1019] uppercase shadow">
              <span id="lightbox-brand-title">{BRAND.name}</span>
            </div>
